...
RSpec.describe 'Event#edit', type: :feature do
  ...
  let!(:notification) { create(:notification, user:, category:) }

  describe 'when user is authenticated' do
    before do
      create(:user_category, user:, category:)
      login_as(user)
      visit edit_events_notification_path(notification.id, locale: I18n.locale)
    end
    ...
    describe 'when notification text is empty' do
      before do
        fill_in 'Notification text', with: nil
        click_button 'Create'
      end
